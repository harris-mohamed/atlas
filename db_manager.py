...
async def seed_officers(officers_dict: dict):
    ...
    async with async_session_maker() as session:
        for officer_id, officer_data in officers_dict.items():
            ...
            result = await session.execute(select(Officer).where(Officer.officer_id == officer_id))
        ...
        for db_officer in all_db_officers.scalars().all():
            if db_officer.officer_id not in officers_dict:
                ...
                print(
                    f"⚠️ Officer {db_officer.officer_id} in DB but not in roster (keeping for history)"
                )
...
async def ensure_channel_exists(channel_id: int, channel_name: str, guild_id: int):
    ...
    async with async_session_maker() as session:
        ...
        if not existing:
            channel = Channel(channel_id=channel_id, channel_name=channel_name, guild_id=guild_id)
            session.add(channel)
            await session.commit()
...
async def add_manual_note(
    channel_id: int, officer_id: str, note_content: str, created_by_user_id: int
) -> bool:
    ...
    async with async_session_maker() as session:
        ...
        result = await session.execute(
            select(ManualNote).where(
                ManualNote.officer_id == officer_id, ManualNote.channel_id == channel_id
            )
        )
...
async def get_channel_stats(channel_id: int) -> Dict:
    ...
    async with async_session_maker() as session:
        ...
        for officer_id in [f"O{i}" for i in range(1, 17)]:
            ...
            stats[officer_id] = {"notes": notes_count, "missions": missions_count}
